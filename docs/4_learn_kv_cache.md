...
3. **单步 Decode vs 批量 Decode**：示例中为了清晰使用了单步（一次 1 个），在实际工程中，Prompt 部分应该一次性放入 Batch 处理（即 `batch.n_tokens = tokens.size()`），这样能充分利用 CPU 的并行计算能力。

现在我们已经能让模型“记住”历史了。但如果我们要实现**高并发**，比如同时处理 10 个用户的对话，该怎么管理 KV Cache 呢？下一篇我们将探讨：**多序列（Multi-sequence）管理与 Batch 优化**。